...
• 超原子时空统-索引*p*, *q*：整合“空间坐标+时间槽”的唯一索引，*p* = (*m* − 1)*N**L* + (*n* − 1)*L* + *l*：*m*（1∼ *M*）、*n*（1  ∼ *N*）为超原子空间行/列坐标:

*l*（1∼ *L*）为时间槽索引；

N为列数，L为时间序列长度

取值范围，整数 1 ≤ *p*, *q* ≤ *N*<sub>*s*</sub>(*N**s* = *M*\**N*\**L* 为时空超原子总数)
...
**旁瓣抑制：**最小化旁瓣区域的平均功率*P*<sub>*s**i**d**e*</sub>（旁瓣会导致能量浪费、通信干扰），旁瓣区域定义为*θ* ∉ $$*θ*<sub>0</sub> − *Δ*, *θ*<sub>0</sub> + *Δ*$$（*Δ**为**旁**瓣**区**域**阈**值*，*通**常**取* 0.1*r**a**d*）；
