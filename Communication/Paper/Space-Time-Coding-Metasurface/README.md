...
• *P*<sub>*s**i**de*</sub>：旁瓣区域平均功率，通过离散角度积分近似计算 —— 将旁瓣区域*θ* ∈ [−*π*/2, *π*/2 ]/ [*θ*<sub>0</sub> − *Δ*, *θ*<sub>0</sub> + *Δ] 离散为K个角度点（实验中*K* = 20）$P_{side}= \frac{1}{K}\sum\_{k = 1}^{K}$ $P_{h}(\theta_{k},\varphi_{0})$
...
•*A*<sub>*pq*</sub><sup>*s*i*d*e</sup>：旁瓣区域的平均时空耦合系数$A_{pq}^{side}= \frac{1}{K}\sum\_{k = 1}^{K}$$A_{pq}(\theta_{k},\varphi_{0})$
