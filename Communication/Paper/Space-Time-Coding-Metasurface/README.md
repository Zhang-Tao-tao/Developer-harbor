...
• 时空耦合系数*A*<sub>*p**q*</sub>：表征超原子*p*与*q*在“空间相位差+时间调制差”下的电磁耦合强度
...
• *P*<sub>*s**i**de*</sub>：旁瓣区域平均功率，通过离散角度积分近似计算 —— 将旁瓣区域*θ* ∈ [−*π*/2, *π*/2 ]/ [*θ*<sub>0</sub> − *Δ*, *θ*<sub>0</sub> + *Δ] 离散为K个角度点（实验中*K* = 20）$P_{\text{side}} = \frac{1}{K}\sum_{k=1}^{K} P_{h}(\theta_{k},\varphi_{0})$
...
•*A*<sub>*pq*</sub><sup>*s*i*d*e</sup>：旁瓣区域的平均时空耦合系数$A_{pq}^{\text{side}} = \frac{1}{K}\sum_{k=1}^{K} A_{pq}(\theta_{k},\varphi_{0})$
...
为实现 “2 比特 4 种相位均匀分布（间隔*π*/2）”，取对称值$a = b = \frac{1}{\sqrt{2}}$，最终：
