...
• *P*<sub>*s**i**de*</sub>：旁瓣区域平均功率，通过离散角度积分近似计算 —— 将旁瓣区域*θ* ∈ \lbrack−*π*/2, *π*/2 \rbrack∖ \lbrack*θ*<sub>0</sub> − *Δ*, *θ*<sub>0</sub> + *Δ*\rbrack 离散为K个角度点（实验中*K* = 20）$P\_{side} = \frac{1}{K}\sum_{k = 1}^{K}P_{h}(\theta_{k},\varphi_{0})$
