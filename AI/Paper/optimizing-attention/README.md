...
- 降低参数量
   通过去除可学习的参数矩阵，以及去除Key矩阵，保留Query和Value矩阵来构建目标函数，从而达到降低参数量的作用，对于Cross-Attention有利。

- 线性化注意力得分求解
   将注意力得分的计算从softmax非线性的操作转换为5次方的线性操作。

- 稀疏性惩罚
   通过对目标函数施加惩罚项，从而增强系数的稀疏性。

- 最优化问题的反向传播
   通过ADMM算法来实现在深度学习层中插入最优化方法的反向传播。这种方法依赖于SVD和反复迭代。
...
**原文创新点。**
首先，对于原文来说，我们认为这篇文章的创新点主要在两方面：

1. 大胆的去除了矩阵K的存在，通过找到Q的分量与V的分量的相似性系数来作为注意力的权重得分，从而减小了参数量。
2. 将注意力得分的计算问题转换为了一个最优化问题，从而能够替代原有的softmax机制。同时，通过转换为最优化问题，能够很自然在局部添加稀疏性惩罚项，而非像以往一样在训练目标函数里全局的添加惩罚。

**复现为QUBO模型。**
为了能够更好的与CIM相干光量子计算进行结合实践，我们进一步的将原文的问题转换为了QUBO问题，有利于在量子退火计算上的实现。为了达成这一目标，首要的就是对系数变量的离散化，这回面临两个问题：首先时必然会损失一些精度，但对于分类问题而言这点损失不会导致太大的问题；同时带来了值域缩放的问题，在复现中我们进行了处理，进行了动态的缩放，解决了此问题。接下来是对目标函数的推导，将其转换为了标准的QUBO模型。我们通过实现验证了转换为QUBO模型后的表达效果，总体评价是有一定的效果，但是不如原版的Attention的效果。另外，我们验证了这个模型是否能够移植到CIM进行真机的计算，并且跑通了全流程。最后对稀疏性进行了分析，发现这个方法确实可以起到让系数更加稀疏的作用。

**原文对于CIM的应用优势**
原文的对于CIM应用的优点在于，它已经把注意力机制的问题转换为了最优化问题，可以很自然的过渡到QUBO。但是，由于原文是实数系数，而QUBO构建出来的是离散变量，因此必定会存在效果的折扣。但是在总体上而言本方法还是有一定的指导性作用的。

**本方法的不足**
在完成了整个模型的QUBO复现之后，我们总结了这个方案存在的一些问题：

1. 计算量较大，本文计算了每个Q和V之间的关系，虽然引入了稀疏性的惩罚，但在总的计算量上是不变的，只是减小了注意力的视野范围，让其专注于更加重要的部分。例如在本实验中，对于一个样本，将图片分割为了49块，那么需要跑49次的CPQC-1(49qubit)。
2. 本方法对于深度学习大模型较为不利，一方面本方法去除了参数，所以在堆叠参数上需要用其他的层来堆叠，没法做到即插即用；另一方面本方法不适合于堆叠的层次结构，因为它的目标在于找到合适的参数让V的加权求和能够更加接近于Q，所以经过一层一层的OptimizingAttention后，Q和V会朝着越来越相似的方向发展，层数越多，意义越小。
