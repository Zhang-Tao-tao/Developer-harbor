...
for idx0 in range(7):
    for idx1 in range(7):
        idx = idx0 * 7 + idx1
        data = json.load(open(f'./cim/QBosonCIMResult/OptimizingAttention{idx}.log', 'r'))
        data = data[0]
...
for idx0 in range(7):
    for idx1 in range(7):
        ...
        ax.text(0.5, -0.1, f'Query-{idx0 * 7 + idx1}', ha='center', va='center', transform=ax.transAxes, fontsize=8)

        
        for i in range(7):
            for j in range(7):
                ax.text(j, 6-i, f'V{i * 7 + j}', ha='center', va='center', color='white', fontsize=6)
