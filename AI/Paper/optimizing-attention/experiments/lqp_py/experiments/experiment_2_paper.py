...
# --- set models:
control_fp = box_qp_control(
    max_iters=10_000, eps_rel=tol, eps_abs=tol, rho=None, adaptive_rho=True, scale=True
)
...
for i in range(n_sims):
    ...
    for model_name in model_names:
        ...
        torch.manual_seed(i)
        k = k + 1
        Q, p, A, b, lb, ub, G, h = create_qp_data(
            n_x, n_batch, n_samples, seed=i, requires_grad=False
        )
        ...
        for epoch in range(n_epochs):
            ...
            loss = (
                0.5
                * torch.matmul(
                    torch.matmul(torch.transpose(z, 1, 2), Q[idx, :, :]), z
                ).sum()
                + (p[idx, :, :] * z).sum()
            )
...
plot_profile_bars(
    backward_times, forward_times, total_times, n_sims, fontsize=8, logy=True
)
...
loss_mean.plot.line(
    ylabel="Normalized QP Loss", xlabel="Epoch", color=color2, linewidth=4
)  # title=title
plt.fill_between(
    np.arange(n_epochs),
    loss_mean["ADMM"] - 2 * loss_error["ADMM"],
    loss_mean["ADMM"] + 2 * loss_error["ADMM"],
    alpha=0.25,
    color=color2[0],
)
plt.fill_between(
    np.arange(n_epochs),
    loss_mean["OptNet"] - 2 * loss_error["OptNet"],
    loss_mean["OptNet"] + 2 * loss_error["OptNet"],
    alpha=0.25,
    color=color2[1],
)
