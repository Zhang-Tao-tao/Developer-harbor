...
for i in range(len(tol)):
    ...
    QP_kkt.append(
        SolveBoxQP(
            control=box_qp_control(eps_rel=tol[i], eps_abs=tol[i], backward="kkt")
        )
    )
    QP_unroll.append(
        SolveBoxQP(control=box_qp_control(eps_rel=tol[i], eps_abs=tol[i], unroll=True))
    )
    QP_optnet.append(
        QPFunction(
            **{
...
models = {
    ...
    "ADMM KKT 3": QP_kkt[1],
    "ADMM KKT 5": QP_kkt[2],
    "ADMM Unroll 1": QP_unroll[0],
    "ADMM Unroll 3": QP_unroll[1],
    "ADMM Unroll 5": QP_unroll[2],
    "OptNet 1": QP_optnet[0],
    "OptNet 3": QP_optnet[1],
    "OptNet 5": QP_optnet[2],
    "SCS 1": QP_scs[0],
    "SCS 3": QP_scs[1],
    "SCS 5": QP_scs[2],
}
