...
cons = [
    A_cvx @ x_cvx == b_cvx,
    G_cvx @ x_cvx <= h_cvx,
    cp.sum_squares(Q_sqrt @ x_cvx) <= z_cvx,
]
problem = cp.Problem(obj, cons)

QP_scs = CvxpyLayer(
    problem,
    parameters=[Q_sqrt, p_cvx, A_cvx, b_cvx, G_cvx, h_cvx],
    variables=[x_cvx, z_cvx],
)  # gp=False
...
for i in range(n_sims):
    ...
    for model_name in model_names:
        ...
        if model_name == "OptNet":
            ...
            start = time.time()
            x = QP_scs(
                torch.linalg.cholesky(Q, upper=True),
                p[:, :, 0],
                A,
                b[:, :, 0],
                G,
...
dat = pd.concat(
    {
        ...
        "Total": total_times.median(axis=0),
    },
    axis=1,
)
